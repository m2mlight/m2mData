/*  Examples of use the m2mData library, when Arduino is connect to Internet, 
    ...
    // Store a "value" of a sensor identified with "api_key" usign http protocol
    void sendValue(char api_key[], float value);  

    // Store a "value" of a sensor identified with "api_key" usign mqtt protocol
...
void setup() {
    ...
    value = 40;
    // Store a "value" of a sensor identified with "api_key" using http protocol
    sensor.sendValue(api_key_sensor, value);

    // Store a "value" of a sensor identified with "api_key" using mqtt protocol
    sensor.sendValueMqtt(api_key_sensor, value);
